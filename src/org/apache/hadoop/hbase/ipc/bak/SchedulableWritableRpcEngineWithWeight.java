...
public class SchedulableWritableRpcEngineWithWeight implements RpcEngine {
  ...
  private static class Invoker implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
      ...
      SchedulableInvocation inv = new SchedulableInvocation(method, protocol, args);
      inv.setWeight(getWeight());
      HbaseObjectWritable value = (HbaseObjectWritable)
        client.call(inv, address,
  ...
  private static ThreadLocal<Integer> WEIGHT;
  ;

  // Here's the default weight
  static {
    WEIGHT = new ThreadLocal<Integer>();
    WEIGHT.set(1);
  }

  public static int getWeight() {
    if (WEIGHT.get() == null) {
      return HBaseScheduleConstant.DEFAULT_HBASE_SCHEDULE_WEIGHT;
    }
    return WEIGHT.get();
  }
  ...
  /** An RPC Server. */
  public static class Server extends SchedulableHBaseServerWithWeight {
    private Object instance;
    private Class<?> implementation;
    private Class<?>[] ifaces;
    private boolean verbose;
    private boolean authorize = false;
    private Map<String, RegionScanner> scanners;
    ...
    public Server(Object instance, final Class<?>[] ifaces,
      ...
      super(bindAddress, port, SchedulableInvocation.class, numHandlers, metaHandlerCount,
          conf, classNameBase(instance.getClass().getName()),
          highPriorityLevel);
      super.setWeightFunction(new WeightFunction(conf));
      ...
      if (instance instanceof HRegionServer) {
        try {
          Field f = instance.getClass().getDeclaredField("scanners");
          f.setAccessible(true);
          this.scanners = (Map<String, RegionScanner>) f.get(instance); //IllegalAccessException
        } catch (Exception e) {
          // TODO Auto-generated catch block
          LOG.error("Failed to get scanners from HRegionServer", e);
          scanners = null;
        }
      }
      else {
     ...
     */
    class WeightFunction implements Function<Writable,Integer> {
      private int defaultWeight;

      public WeightFunction(Configuration conf) {
        defaultWeight = conf.getInt(HBaseScheduleConstant.HBASE_SCHEDULE_WEIGHT
          , HBaseScheduleConstant.DEFAULT_HBASE_SCHEDULE_WEIGHT);
      }

      @Override
      public Integer apply(Writable from) {
        if (scanners != null) {
          if (!(from instanceof Invocation)) return defaultWeight;
          Invocation inv = (Invocation) from;

          if (inv instanceof SchedulableInvocation) {
            LOG.info("Invocation is schedulable.");
            SchedulableInvocation sInv = (SchedulableInvocation) inv;
            return sInv.getWeight();
          } else {
