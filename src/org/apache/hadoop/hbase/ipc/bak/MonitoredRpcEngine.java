...
class MonitoredRpcEngine implements RpcEngine {
  ...
  private static class Invoker implements InvocationHandler {
    ...
    public Invoker(HBaseClient client, Class<? extends VersionedProtocol> protocol,
        InetSocketAddress address, User ticket, Configuration conf, int rpcTimeout) {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      HbaseObjectWritable value =
          (HbaseObjectWritable) client.call(new Invocation(method, protocol, args), address,
  ...
  public <T extends VersionedProtocol> T getProxy(Class<T> protocol, long clientVersion,
    ...
    T proxy =
        (T) Proxy.newProxyInstance(
          ...
          new Class[] { protocol },
          new Invoker(client, protocol, addr, userProvider.getCurrent(), conf, HBaseRPC
              .getRpcTimeout(rpcTimeout)));
     ...
     */
    long serverVersion =
        ((VersionedProtocol) proxy).getProtocolVersion(protocol.getName(), clientVersion);
  ...
  public Server getServer(Class<? extends VersionedProtocol> protocol, Object instance,
    ...
    return new Server(instance, ifaces, conf, bindAddress, port, numHandlers, metaHandlerCount,
        verbose, highPriorityLevel);
  }

  /** An RPC Server. */
  public static class Server extends HBaseServer implements Schedulable {
    ...
    private static final String QUEUE_DUMP_INTERVAL = "hbase.ipc.queue.dump.interval";
    private static final long DEFAULT_QUEUE_DUMP_INTERVAL = 10 * 1000;

    private static final String QUEUE_DUMP_ZNODE = "hbase.ipc.queue.dump.znode";
    private static final String QUEUE_DUMP_ZNODE_DEDAULT = "/hbase_monitored";
    ...
    public Server(Object instance, final Class<?>[] ifaces, Configuration conf, String bindAddress,
        int port, int numHandlers, int metaHandlerCount, boolean verbose, int highPriorityLevel)
        throws IOException {
      ...
      LOG.info("Initialize monitored rpc server.");
      ...
      if (oldMaxQueueSize == null) {
        this.maxQueueLength = this.conf.getInt("ipc.server.max.callqueue.length", numHandlers * 10);
      } else {
    ...
    public void stop() {
      ...
      if (monitor != null) {
        ...
        try {
          ...
          LOG.error("Error ocuppies when closing monitor thread.", e);
        }
      }
      super.stop();
    }
    ...
    public Writable call(Class<? extends VersionedProtocol> protocol, Writable param,
        long receivedTime, MonitoredRPCHandler status) throws IOException {
      try {
        ...
        if (!method.getDeclaringClass().equals(VersionedProtocol.class)) {
          ...
          ProtocolSignature serverInfo =
              ((VersionedProtocol) instance).getProtocolSignature(protocol.getCanonicalName(),
                call.getProtocolVersion(), call.getClientMethodsHash());
  ...
  protected static void log(String value) {
    String v = value;
    if (v != null && v.length() > 55) v = v.substring(0, 55) + "...";
    LOG.info(v);
  }
}
